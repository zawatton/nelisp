...
| User native units: `nelisp-native-unit-rebuild-and-reload`, `-stage`, `-publish`, `-address` | `lisp/nelisp-native-unit.el`, `-development.el`; stable entry gates and single-use CAS publication; [REPL procedure](repl-development.md#replace-user-defined-native-functions) | Closed raw-v1 integer SysV units, Linux x86_64 only; common reload adapter not integrated; existing executable direct calls unchanged; old mappings retained |
