;;; nelisp-coding-utf8.el --- Minimal self-hosted UTF-8 codec  -*- lexical-binding: t; -*-

;;; Commentary:

;; Pure NeLisp UTF-8 support shared by the standalone loader and the full
;; coding layer.  This module deliberately has no Latin-1 or JIS table
;; dependency so source loading does not pull the full coding subsystem into
   ...
   ((stringp bytes)
    ...
    (let ((c (aref bytes pos)))
      (if (and (fboundp 'multibyte-string-p)
               (funcall 'multibyte-string-p bytes))
          (logand c #xFF)
        c)))
  ...
  (cond
   ...
   ((stringp bytes)
    (if (and (fboundp 'multibyte-string-p)
        ...
        (append (funcall 'encode-coding-string bytes 'utf-8 t) nil)
      (append bytes nil)))
   (t (signal 'wrong-type-argument (list 'sequencep bytes)))))
...
(provide 'nelisp-coding-utf8)
;;; nelisp-coding-utf8.el ends here
