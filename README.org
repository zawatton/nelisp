...
#+begin_src text
  ...
  ├──────────────────────────────────────────────────┤
  │ nelisp-runtime crate                             │  pure NeLisp runtime
  │   bridge/ + eval/ + reader/ + sqlite + syscall   │
  ├──────────────────────────────────────────────────┤
  │ Elisp evaluator (Rust bootstrap layer)           │  Phase 8.0.2
  │   25 special forms + 75 builtins                 │
  ├──────────────────────────────────────────────────┤
  │ Elisp reader (Rust bootstrap layer)              │  Phase 8.0.1
  │   sexp / lexer / parser                          │
  ├──────────────────────────────────────────────────┤
  │ OS substrate                                     │  Phase 7.0 / 9d
  │   syscall wrappers; allocator/GC/coding in Lisp  │
  └──────────────────────────────────────────────────┘
#+end_src

Doc 46 (=architecture α boundary final=) is the current boundary:
NeLisp owns no =anvil_*= or =mcp= modules.  The optional MCP front end
lives in the sibling =anvil-runtime= crate and calls into
=nelisp-runtime= through a path dependency.
