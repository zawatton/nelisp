#!/usr/bin/env bash
# release/stage-d-v3.0/install-v3.sh — v1.3.1 standalone installer

set -euo pipefail

RELEASE_VERSION="${RELEASE_VERSION:-v1.3.1}"
...
usage() {
  cat <<USAGE
NeLisp standalone installer (v1.3.1 default; true standalone, no Emacs runtime dependency)
