...
excluded."
  (cl-case (nelisp-sys-ast-kind node)
    ...
    (char-table-get (list (nelisp-sys-ast-prop node :table)
                          (nelisp-sys-ast-prop node :index)
                          (nelisp-sys-ast-prop node :out)))
    (char-table-set! (list (nelisp-sys-ast-prop node :table)
                           (nelisp-sys-ast-prop node :index)
                           (nelisp-sys-ast-prop node :value)
                           (nelisp-sys-ast-prop node :out)))
    (mut-str-make-empty (list (nelisp-sys-ast-prop node :slot)
                              (nelisp-sys-ast-prop node :cap)))
    (mut-str-push-byte (list (nelisp-sys-ast-prop node :ptr)
                             (nelisp-sys-ast-prop node :byte)))
    (mut-str-push-codepoint (list (nelisp-sys-ast-prop node :ptr)
                                  (nelisp-sys-ast-prop node :codepoint)))
    (mut-str-len (list (nelisp-sys-ast-prop node :ptr)))
    (mut-str-finalize (list (nelisp-sys-ast-prop node :ptr)
                            (nelisp-sys-ast-prop node :slot)))
    ((f64-arith f64-cmp) (nelisp-sys-ast-prop node :args))
    ((i64->f64 f64->i64 bits->f64) (list (nelisp-sys-ast-prop node :arg)))
    (str-to-float (list (nelisp-sys-ast-prop node :bytes)
                        (nelisp-sys-ast-prop node :len)
                        (nelisp-sys-ast-prop node :out)))
    (t nil)))
