...
unibyte-string, patch in place with plain `aset', then store back
as a single chunk (= the cached chunk list collapses to length 1
so subsequent `buffer-bytes' calls remain O(total-bytes)).

Every :chunks entry is produced by `unibyte-string' — directly, or
via `nelisp-asm-x86_64--byte-vec->string's small-arity `concat' of
`unibyte-string' pieces — and `--append-bytes' only ever `concat's
those unibyte strings together, so the materialized buffer is a
unibyte string end to end, on host Emacs AND the standalone
runtime: `multibyte-string-p' is nil, `length' already answers the
byte length, and `aset'/`aref' read and write raw bytes with no
UTF-8 decode/encode in the way.  Verified directly against a built
`target/nelisp': `(let ((s (copy-sequence (unibyte-string 1 2 3 4 5
6 7 8)))) (aset s 2 200) (aset s 3 255) (aset s 4 128) s)' keeps
`multibyte-string-p' nil and reads back the exact bytes (including
values that would form a multi-byte UTF-8 sequence if decoded),
identical to host; likewise `concat' of two `unibyte-string's.  So
this asserts unibyte-ness up front instead of assuming it, then
patches the materialized string directly — no `--byte-at' /
`--byte-vec->string' round trip, no `make-vector', no per-byte loop
of any kind, scoped by chunk or otherwise.  An earlier version of
this function still ran that round trip per chunk that carried a
fixup; on a real unit with many fixups (most chunks) that was
barely better than copying the whole buffer, since it still touched
every fixup-bearing byte through the interpreted accessor rather
than the native `aset'."
  (let* ((bytes (apply #'concat (nreverse (copy-sequence (aref buf 0)))))
         (labels (aref buf 2))
         (fixups (aref buf 3)))
    (when (multibyte-string-p bytes)
      ;; Every chunk is built from `unibyte-string' (see above), so this
      ;; would mean some caller fed a multibyte piece into the buffer —
      ;; fail loudly rather than silently corrupt the patch below, which
      ;; indexes BYTES not characters.
      (signal 'nelisp-asm-x86_64-error
              (list :unexpected-multibyte-buffer (length bytes))))
    (dolist (fix fixups)
      (let* ((slot (car fix))
             (label (cdr fix))
        ...
        (let* ((rel32 (- (cdr cell) (+ slot 4)))
          ...
          (aset bytes    slot      (logand u #xFF))
          (aset bytes (+ slot 1)   (logand (ash u  -8) #xFF))
          (aset bytes (+ slot 2)   (logand (ash u -16) #xFF))
          (aset bytes (+ slot 3)   (logand (ash u -24) #xFF)))))
    ;; Collapse chunk list to a single materialized chunk so
    ;; subsequent `buffer-bytes' calls return the patched form.
    (aset buf 0 (list bytes))
    bytes))
