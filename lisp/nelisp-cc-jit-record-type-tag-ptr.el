...
;; `sexp-payload-ptr(arg)' returns the NlRecord box pointer which IS
;; `*const Sexp' for the inline `type_tag' field --- no arithmetic needed.
;;
;; On non-Record variants `sexp-payload-ptr' returns 0 for unboxed
;; variants (Nil/T/Int/Float) but may return a non-null garbage pointer
;; for other boxed variants.  The explicit tag guard ensures we return 0
;; (null) for any non-Record input, matching the Rust body semantics.
;;
...
(defconst nelisp-cc-jit-record-type-tag-ptr--source
  (quote (defun nl_record_type_tag_ptr (arg)
     ...
     ;;
     ;; sexp-payload-ptr reads the NonNull<NlRecord> box pointer
     ;; stored at SEXP_PAYLOAD_OFFSET=8 inside the Sexp enum.
