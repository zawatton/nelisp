...
;; Doc 100 §100.D — AOT-compiled replacements for the 12 plain
...
;; `nelisp-aot-compile-to-object', producing an ET_REL `.o' file
...
;; until ARM64 / non-Linux AOT codegen lands (= Stage 2 of the
;; arith.rs swap).
...
(defconst nelisp-cc-jit-arith-sub2--source
  ...
  "AOT source for `nl_jit_arith_sub2' — wrapping i64 subtract.")
...
(defconst nelisp-cc-jit-arith-ash--source
  '(defun nelisp_jit_ash (n count)
     (if (< count 0)
         (sar n (- 0 count))
       (shl n count)))
  "AOT source for `nl_jit_arith_ash' — signed arithmetic shift.
