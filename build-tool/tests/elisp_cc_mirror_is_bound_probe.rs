...
fn run_is_bound(mirror: &Sexp, sym: &Sexp, unbound: &Sexp) -> i64 {
    unsafe {
        nelisp_build_tool::elisp_cc_spike::mirror_is_bound(
...
#[test]
fn mirror_is_bound_positive_value_returns_1() {
    let mirror = build_empty_mirror(1024);
    let unbound = Sexp::Symbol("nelisp--unbound-marker".into());
    install_entry(&mirror, "alpha", Sexp::Int(42), unbound.clone());

    let sym = Sexp::Symbol("alpha".into());
    assert_eq!(run_is_bound(&mirror, &sym, &unbound), 1);
}

#[test]
fn mirror_is_bound_negative_unbound_returns_0() {
    let mirror = build_empty_mirror(1024);
    let unbound = Sexp::Symbol("nelisp--unbound-marker".into());
    // Entry exists but slot 0 = unbound-marker.
    install_entry(&mirror, "alpha", unbound.clone(), unbound.clone());

    let sym = Sexp::Symbol("alpha".into());
    assert_eq!(run_is_bound(&mirror, &sym, &unbound), 0);
}

#[test]
fn mirror_is_bound_edge_absent_entry_returns_0() {
    let mirror = build_empty_mirror(1024);
    let unbound = Sexp::Symbol("nelisp--unbound-marker".into());
...
#[test]
fn mirror_is_bound_non_symbol_value_returns_1() {
    // `symbol-eq' returns 0 for any tag-mismatched input; a value cell
    // holding `Sexp::Cons' / `Sexp::Vector' / etc. must still classify
    // as bound.
    let mirror = build_empty_mirror(1024);
    let unbound = Sexp::Symbol("nelisp--unbound-marker".into());
    install_entry(
    ...
    let sym = Sexp::Symbol("list-binding".into());
    assert_eq!(run_is_bound(&mirror, &sym, &unbound), 1);
}
