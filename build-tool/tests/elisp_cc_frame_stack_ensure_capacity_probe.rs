...
fn frame_stack_ensure_capacity_no_grow_when_already_sufficient() {
    ...
    let got = unsafe {
        nelisp_build_tool::elisp_cc_spike::frame_stack_ensure_capacity(&frames as *const Sexp, 4)
    };
    ...
    assert!(
        got > 0,
        "no-grow path must return a positive value, got {}",
        got
    );
}
...
fn frame_stack_ensure_capacity_grows_to_next_pow2() {
    let frames = build_empty_frames_record(8);
    let got = unsafe {
        nelisp_build_tool::elisp_cc_spike::frame_stack_ensure_capacity(&frames as *const Sexp, 16)
    };
    // Grow doubles to 16.
    ...
    assert!(
        got >= 16,
        "grow path must return new capacity >= needed, got {}",
        got
    );
}
...
fn frame_stack_ensure_capacity_grows_multiple_doublings() {
    let frames = build_empty_frames_record(8);
    unsafe {
        nelisp_build_tool::elisp_cc_spike::frame_stack_ensure_capacity(&frames as *const Sexp, 100);
    }
