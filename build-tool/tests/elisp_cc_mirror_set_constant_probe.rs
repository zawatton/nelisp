...
fn run_set_constant(mirror: &Sexp, sym: &Sexp, flag: &Sexp) -> i64 {
    unsafe {
        nelisp_build_tool::elisp_cc_spike::mirror_set_constant(
...
#[test]
fn mirror_set_constant_positive_hit_writes_t_into_slot_3() {
    let mirror = build_empty_mirror(1024);
    install_entry(&mirror, "c", Sexp::Int(0), Sexp::Nil);
    let sym = Sexp::Symbol("c".into());
    let t = Sexp::T;
    assert_eq!(run_set_constant(&mirror, &sym, &t), 1);
    assert_eq!(read_slot(&mirror, "c", 3).unwrap(), Sexp::T);
}

#[test]
fn mirror_set_constant_negative_miss_returns_zero() {
    let mirror = build_empty_mirror(1024);
    install_entry(&mirror, "x", Sexp::Int(0), Sexp::Nil);
    let sym = Sexp::Symbol("absent".into());
    let flag = Sexp::T;
    assert_eq!(run_set_constant(&mirror, &sym, &flag), 0);
}

#[test]
fn mirror_set_constant_edge_empty_mirror_returns_zero() {
    let mirror = build_empty_mirror(1024);
...
#[test]
fn mirror_set_constant_toggle_t_to_nil() {
    // Two sequential set_constant calls: first sets T, then Nil.
    // Both must hit on the existing entry.
    let mirror = build_empty_mirror(1024);
    install_entry(&mirror, "c", Sexp::Int(0), Sexp::Nil);
    ...
    let nil = Sexp::Nil;
    assert_eq!(run_set_constant(&mirror, &sym, &nil), 1);
    assert_eq!(read_slot(&mirror, "c", 3).unwrap(), Sexp::Nil);
}
