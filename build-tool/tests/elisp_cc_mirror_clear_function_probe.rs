...
fn run_clear_function(mirror: &Sexp, sym: &Sexp, unbound: &Sexp) -> i64 {
    unsafe {
        nelisp_build_tool::elisp_cc_spike::mirror_clear_function(
...
#[test]
fn mirror_clear_function_positive_hit_sets_slot_1_to_unbound() {
    let mirror = build_empty_mirror(1024);
    install_entry(&mirror, "f", Sexp::Nil, Sexp::Symbol("car".into()));
    let sym = Sexp::Symbol("f".into());
    let unbound = Sexp::Symbol("nelisp--unbound-marker".into());
    assert_eq!(run_clear_function(&mirror, &sym, &unbound), 1);
    assert_eq!(read_slot(&mirror, "f", 1).unwrap(), unbound);
}

#[test]
fn mirror_clear_function_negative_miss_returns_zero() {
    let mirror = build_empty_mirror(1024);
    install_entry(&mirror, "x", Sexp::Nil, Sexp::Symbol("fn".into()));
    let sym = Sexp::Symbol("absent".into());
    let unbound = Sexp::Symbol("nelisp--unbound-marker".into());
    assert_eq!(run_clear_function(&mirror, &sym, &unbound), 0);
    assert_eq!(read_slot(&mirror, "x", 1).unwrap(), Sexp::Symbol("fn".into()));
}

#[test]
fn mirror_clear_function_edge_empty_mirror_returns_zero() {
    let mirror = build_empty_mirror(1024);
...
#[test]
fn mirror_clear_function_does_not_touch_value_slot() {
    let mirror = build_empty_mirror(1024);
    install_entry(&mirror, "k", Sexp::Int(99), Sexp::Symbol("g".into()));
    let sym = Sexp::Symbol("k".into());
    let unbound = Sexp::Symbol("nelisp--unbound-marker".into());
    assert_eq!(run_clear_function(&mirror, &sym, &unbound), 1);
    assert_eq!(read_slot(&mirror, "k", 0).unwrap(), Sexp::Int(99));
    assert_eq!(read_slot(&mirror, "k", 1).unwrap(), unbound);
}
