...
fn alloc_bytes_then_write_read_then_dealloc() {
    ...
    let ptr = unsafe { nelisp_build_tool::elisp_cc_spike::alloc_bytes(size, align) };
    assert!(
        !ptr.is_null(),
    ...
    unsafe {
        let head_rc =
            nelisp_build_tool::elisp_cc_spike::ptr_write_u64(ptr, 0, 0x0123_4567_89AB_CDEF_i64);
        assert_eq!(head_rc, 1);
    ...
    let head = unsafe { nelisp_build_tool::elisp_cc_spike::ptr_read_u64(ptr as *const u8, 0) };
    let tail = unsafe { nelisp_build_tool::elisp_cc_spike::ptr_read_u64(ptr as *const u8, 56) };
    ...
    let dealloc_rc = unsafe { nelisp_build_tool::elisp_cc_spike::dealloc_bytes(ptr, size, align) };
    assert_eq!(dealloc_rc, 1, "dealloc-bytes must return rax = 1 sentinel");
}
...
fn alloc_bytes_multiple_sizes_each_dealloced() {
    ...
    for &size in &sizes {
        let ptr = unsafe { nelisp_build_tool::elisp_cc_spike::alloc_bytes(size, align) };
        assert!(!ptr.is_null(), "alloc-bytes({}, 8) must succeed", size);
        ...
        unsafe {
            ...
            nelisp_build_tool::elisp_cc_spike::ptr_write_u64(ptr, size - 8, size + 1);
        }
    ...
    for (i, &(ptr, size)) in allocations.iter().enumerate() {
        ...
        let tail =
            unsafe { nelisp_build_tool::elisp_cc_spike::ptr_read_u64(ptr as *const u8, size - 8) };
        assert_eq!(
            tail,
            size + 1,
    ...
    for &(ptr, size) in allocations.iter().rev() {
        let rc = unsafe { nelisp_build_tool::elisp_cc_spike::dealloc_bytes(ptr, size, align) };
...
fn alloc_bytes_honors_requested_alignment() {
    ...
    let ptr16 = unsafe { nelisp_build_tool::elisp_cc_spike::alloc_bytes(128, 16) };
    ...
    let ptr32 = unsafe { nelisp_build_tool::elisp_cc_spike::alloc_bytes(256, 32) };
    ...
    unsafe {
        let rc16 = nelisp_build_tool::elisp_cc_spike::dealloc_bytes(ptr16, 128, 16);
        assert_eq!(rc16, 1);
        let rc32 = nelisp_build_tool::elisp_cc_spike::dealloc_bytes(ptr32, 256, 32);
        assert_eq!(rc32, 1);
    }
}
