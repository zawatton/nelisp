...
#[test]
fn nlvector_clone_single_increments_and_returns_input_ptr() {
    assert_eq!(
        std::mem::offset_of!(ProbeBox, refcount),
        24,
        "ProbeBox layout must mirror NlVector (= refcount at +24)"
    );

    let bx = ProbeBox::new(1);
    let input_ptr = bx.as_box_ptr();
    let returned_ptr = unsafe { nelisp_build_tool::elisp_cc_spike::nlvector_clone(input_ptr) };
    assert_eq!(
        returned_ptr as usize, input_ptr as usize,
        "nelisp_nlvector_clone must return the *input* pointer unchanged \
...
#[test]
fn nlvector_clone_n_times_count_equals_n_plus_initial() {
    const INITIAL: i64 = 1;
    const N: usize = 10;
    ...
    let input_ptr = bx.as_box_ptr();
    for i in 0..N {
        let returned = unsafe { nelisp_build_tool::elisp_cc_spike::nlvector_clone(input_ptr) };
        assert_eq!(
...
#[test]
fn nlvector_clone_concurrent_no_lost_updates() {
    use std::sync::Arc;
    use std::thread;
    ...
    let t1 = thread::spawn(move || {
        ...
        for _ in 0..PER_THREAD {
            let _ret = unsafe { nelisp_build_tool::elisp_cc_spike::nlvector_clone(p) };
        }
    });
