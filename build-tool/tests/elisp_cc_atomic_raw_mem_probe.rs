...
fn atomic_fetch_add_zero_slot_returns_zero_and_writes_delta() {
    let mut slot: i64 = 0;
    let prev = unsafe {
        nelisp_build_tool::elisp_cc_spike::atomic_fetch_add(&mut slot as *mut i64, 7)
    };
    assert_eq!(
        prev, 0,
...
fn atomic_fetch_add_existing_slot_returns_old_and_accumulates() {
    ...
    let prev = unsafe {
        nelisp_build_tool::elisp_cc_spike::atomic_fetch_add(&mut slot as *mut i64, -3)
    };
    ...
    let prev2 = unsafe {
        nelisp_build_tool::elisp_cc_spike::atomic_fetch_add(&mut slot as *mut i64, 1)
    };
    assert_eq!(prev2, 97);
    assert_eq!(slot, 98);
}
...
fn atomic_compare_exchange_success_returns_one_and_writes() {
    let mut slot: i64 = 42;
    let rc = unsafe {
        nelisp_build_tool::elisp_cc_spike::atomic_compare_exchange(
            &mut slot as *mut i64,
            42,
            999,
        )
...
fn ptr_read_write_u64_round_trip() {
    ...
    assert_eq!(
        write_rc, 1,
        "ptr-write-u64 must return rax = 1 sentinel"
    );
    assert_eq!(
    ...
    let read = unsafe {
        nelisp_build_tool::elisp_cc_spike::ptr_read_u64(base as *const u8, 16)
    };
    assert_eq!(
        read as u64,
...
fn ptr_read_write_u8_round_trip_with_zero_extension() {
    ...
    let write_rc = unsafe {
        nelisp_build_tool::elisp_cc_spike::ptr_write_u8(base, 3, 0xFF)
    };
    assert_eq!(write_rc, 1, "ptr-write-u8 must return rax = 1 sentinel");
    assert_eq!(buf[3], 0xFF, "byte 3 must hold the written value");

    let read = unsafe {
        nelisp_build_tool::elisp_cc_spike::ptr_read_u8(base as *const u8, 3)
    };
    assert_eq!(
        read, 0xFF,
    ...
    let r2 = unsafe {
        nelisp_build_tool::elisp_cc_spike::ptr_read_u8(base as *const u8, 7)
    };
    assert_eq!(r2, 0x42);
}
