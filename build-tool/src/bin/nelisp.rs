...
//! Rust API.
//!
//! Stage 9a-9g course correction (2026-04-30): the previous
//! incarnation of this file grew ~700 LOC of mint-* command handlers
//! that duplicated the existing self-hosted Elisp native compiler
//! (src/nelisp-cc-*.el).  Per user principle "Rust core 最小化、
//! Elisp で書ける物は全て Elisp で書く" (memory
//! `feedback_rust_core_minimization_principle'), the 9a-9g
//! handlers (mint-int-as-code, mint-int-via-emitted-load,
//! mint-int-plus-imm, mint-chain, mint-from-ast) have been removed.
//! Their replacement is the Elisp-side `lisp/doc47-mint.el' driven
//! via `emacs --batch'.
//!
//! Surviving Stage 6/7a/8 commands (mint-list/string/symbol-from-source,
//! mint-eval-result, mint-eval-file) remain because they are simple
//! lowering operations that ride the existing image_lowering.rs
//! substrate; they will be ported to Elisp in a follow-up sweep.
...
Stage 9a-9g handlers (mint-int-as-code, mint-int-via-emitted-load,
mint-int-plus-imm, mint-chain, mint-from-ast) were removed in the
2026-04-30 Elisp-pivot rollback.  Use the Elisp driver at
lisp/doc47-mint.el via `emacs --batch' instead.";
...
fn main() -> ExitCode {
    ...
    match command {
        ...
        Command::MintListFromSource { src, out } => run_mint_with_asset(
            &src,
            &out,
            "list-from-source",
            NATIVE_LIST_LENGTH,
        ...
        Command::MintStringFromSource { src, out } => run_mint_with_asset(
            &src,
            &out,
            "string-from-source",
        ...
        Command::MintSymbolFromSource { src, out } => run_mint_with_asset(
            &src,
            &out,
            "symbol-from-source",
        ...
        Command::MintEvalFile { src_file, out } => match fs::read_to_string(Path::new(&src_file)) {
...
mod tests {
    ...
    fn pick_asset_classifies_each_supported_shape() {
        ...
        assert_eq!(pick_asset_for_eval_result(&Sexp::Int(7)).unwrap().2, "int-or-t");
        assert_eq!(pick_asset_for_eval_result(&Sexp::T).unwrap().2, "int-or-t");
        assert_eq!(pick_asset_for_eval_result(&Sexp::Nil).unwrap().2, "list");
        assert_eq!(
            pick_asset_for_eval_result(&Sexp::cons(Sexp::Int(1), Sexp::Nil)).unwrap().2,
            "list"
        );
        assert_eq!(pick_asset_for_eval_result(&Sexp::Str("hi".into())).unwrap().2, "string");
        assert_eq!(pick_asset_for_eval_result(&Sexp::Symbol("foo".into())).unwrap().2, "symbol");
        assert_eq!(pick_asset_for_eval_result(&Sexp::Float(3.14)).unwrap().2, "float-trunc");
