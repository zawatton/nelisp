...
pub fn read_str(input: &str) -> Result<Sexp, ReadError> {
    let mut state = ElispReadState::new(input); let form = match state.parse_one_form() { Some(ElispParseOutcome::Form(f)) => f, Some(ElispParseOutcome::Empty) => return Err(ReadError::UnexpectedEof("empty input".into(), START_POS)), None => return Err(ReadError::NotYetImplemented(NYI_MSG.into(), START_POS)) };
    if state.has_trailing_token() { return Err(ReadError::Parse("trailing token after first form".into(), START_POS)); } Ok(form)
}
pub fn read_all(input: &str) -> Result<Vec<Sexp>, ReadError> {
    let mut state = ElispReadState::new(input); let mut forms = Vec::new();
    loop { match state.parse_one_form() { Some(ElispParseOutcome::Form(form)) => forms.push(form), Some(ElispParseOutcome::Empty) => return Ok(forms), None => return Err(ReadError::NotYetImplemented(NYI_MSG.into(), START_POS)) } }
}
