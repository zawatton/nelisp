...
mod tests {
    ...
    #[test] fn call_i64_i64_smoke() {
        ...
        assert!(err(&bi_nl_jit_call_i64_i64(&[s("nelisp_jit_no_such"),i(0),i(0)]).unwrap_err(), "error"));
        assert!(err(&bi_nl_jit_call_i64_i64(&[s("nelisp_jit_add2"),i(1)]).unwrap_err(), "wrong-number-of-arguments"));
        assert!(err(&bi_nl_jit_call_i64_i64(&[i(0),i(1),i(2)]).unwrap_err(), "wrong-type-argument"));
    ...
    #[test] fn call_syscall_errors() {
        assert!(err(&bi_nl_jit_call_syscall(&vec![s("nelisp_jit_syscall");7]).unwrap_err(), "wrong-number-of-arguments"));
        let mut a = vec![s("nelisp_jit_no_syscall")]; a.extend(std::iter::repeat(i(0)).take(7));
        assert!(err(&bi_nl_jit_call_syscall(&a).unwrap_err(), "error")); }
