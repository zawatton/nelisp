...
mod tests {
    use super::*;
    unsafe fn call(nr: i64, a0: i64, a1: i64, a2: i64) -> i64 {
        nl_jit_syscall_call(nr, a0, a1, a2, 0, 0, 0)
    }
    #[test]
    ...
    fn jit_syscall_getpid_matches_libc() {
        let r = unsafe { call(libc::SYS_getpid as i64, 0, 0, 0) };
        assert_eq!(r, unsafe { libc::getpid() } as i64);
        assert!(r > 0);
    }
    #[test]
