...
extern "C" {
    ...
    fn nl_sf_lambda();
    fn nl_sf_function();
    // Phase 47 elisp migration: `nl_jit_string_match_p' — string-match-p
    // trampoline; Rust body deleted from `jit/regex.rs'.
    fn nl_jit_string_match_p();
}

/// Keep the archive symbols live through LTO.
#[used]
static _ELISP_ARCHIVE_ANCHOR: [unsafe extern "C" fn(); 62] = [
