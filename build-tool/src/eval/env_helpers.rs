...
impl Env {
    ...
    pub fn capture_lexical(&mut self) -> Sexp {
        let Sexp::Record(r) = &self.frames_record else { return Sexp::Nil }; let (Some(Sexp::Int(d)), Ok(f)) = (r.slots.get(1), self.lookup_function("nelisp-lexframe-stack-capture-to-depth")) else { return Sexp::Nil };
    ...
    pub(crate) fn frame_pop_rust_direct(&mut self) {
        let Sexp::Record(sr) = &self.frames_record else { return }; let sr = sr.clone(); let (Some(Sexp::Vector(bk)), Some(Sexp::Int(d))) = (sr.slots.get(0), sr.slots.get(1)) else { return };
    ...
    pub fn frame_stack_find_rust_direct(&self, name: &str) -> Option<Sexp> {
        if !matches!(&self.frames_record, Sexp::Record(_)) { return None; } let raw = unsafe { crate::elisp_cc_spike::frame_stack_find_raw(&self.frames_record, &Sexp::Str(name.to_string())) }; if raw.is_null() { None } else { Some(unsafe { (*raw).clone() }) } }
