...
mod tests {
    use super::*;
    #[test] fn alloc_dealloc_bytes_round_trip() {
        let (size, align) = (64i64, 8i64);
        let ptr = unsafe { nl_alloc_bytes(size, align) };
        assert!(!ptr.is_null()); unsafe { nl_dealloc_bytes(ptr, size, align) };
    }
    #[test] fn alloc_bytes_rejects_bad_alignment() {
        assert!(unsafe { nl_alloc_bytes(32, 3) }.is_null()); assert!(unsafe { nl_alloc_bytes(0, 8) }.is_null());
        assert!(unsafe { nl_alloc_bytes(-1, 8) }.is_null()); assert!(unsafe { nl_alloc_bytes(32, 0) }.is_null());
    }
    #[test] fn dealloc_bytes_null_is_no_op() { unsafe { nl_dealloc_bytes(std::ptr::null_mut(), 32, 8) }; }
    #[test] fn alloc_bytes_16_byte_aligned() {
        let ptr = unsafe { nl_alloc_bytes(128, 16) };
        assert!(!ptr.is_null()); assert_eq!((ptr as usize) & 0xF, 0);
