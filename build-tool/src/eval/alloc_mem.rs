...
mod tests {
    ...
    #[test] fn alloc_dealloc_null_and_aligned() { unsafe{nl_dealloc_bytes(std::ptr::null_mut(),32,8)}; let p=unsafe{nl_alloc_bytes(128,16)}; assert!(!p.is_null()&&(p as usize)&0xF==0); unsafe{nl_dealloc_bytes(p,128,16)} }
}
