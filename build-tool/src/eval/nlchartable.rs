...
fn ct_set(i: &mut CharTableInner, c: i64, v: Sexp) { match i.entries.iter_mut().find(|(k,_)| *k==c) { Some(e) => e.1=v, None => i.entries.push((c,v)) } }
fn ct_get(rc: &NlCharTableRef, c: i64) -> Sexp { rc.inner.entries.iter().find(|(k,_)| *k==c).map(|(_,v)| v.clone()).or_else(|| rc.inner.parent.as_ref().map(|p| ct_get(p,c))).unwrap_or_else(|| rc.inner.default_val.clone()) }
