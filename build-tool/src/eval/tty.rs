...
pub fn raw_mode_enter() -> Result<(), EvalError> { match unsafe{crate::elisp_cc_spike::tty_raw_enter([0u8;60].as_mut_ptr())}{0=>Ok(()),-1=>Err(EvalError::internal("terminal-raw-mode-enter: tcgetattr failed")),_=>Err(EvalError::internal("terminal-raw-mode-enter: tcsetattr failed"))} }
pub fn raw_mode_leave() -> Result<(), EvalError> { unsafe{crate::elisp_cc_spike::tty_raw_leave(nl_tty_saved_termios_ptr())}; Ok(()) }
pub fn stdin_byte_available(t: i32) -> Result<Option<u8>, EvalError> { match unsafe{crate::elisp_cc_spike::tty_stdin_byte_avail([0u8,0,0,0,1,0,0,0].as_mut_ptr(),t as i64)}{-2=>Err(EvalError::internal("read-stdin-byte-available: poll failed")),n if n<0=>Ok(None),b=>Ok(Some(b as u8))} }
