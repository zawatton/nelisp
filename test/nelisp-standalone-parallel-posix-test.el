...
(ert-deftest nelisp-standalone-parallel-posix-macos-os-compat-script ()
  ...
  (let* ((script-path
    ...
    (should (string-match-p "test/nelisp-stdlib-os-test.el" script))
    (should (string-match-p "EXPECTED_COUNT=36" script))
    (should (string-match-p "signals" script))
    (should (string-match-p "signalfd-darwin" script))
    (should (string-match-p "sigprocmask-darwin" script))
