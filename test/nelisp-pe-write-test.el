...
(defun nelisp-pe-write-test--read-c-string (bytes offset)
  "Read a NUL-terminated byte string from BYTES at OFFSET."
  (let ((end offset))
...
(ert-deftest nelisp-pe-write-exe-binary-winsock-socket-section-table ()
  "The Winsock socket smoke EXE has .data WSADATA bytes and multi-DLL .idata."
  (let* ((bytes (nelisp-pe-write-test--emit-exe 'winsock-socket-exit-42))
         (pe-off (nelisp-pe-write-test--read-le32 bytes #x3c))
         (file-off (+ pe-off 4))
         (opt-off (+ file-off 20))
         (sect0 (+ pe-off 4 20 240))
...
(ert-deftest nelisp-pe-write-exe-binary-winsock-socket-import-directory ()
  "The Winsock socket smoke EXE imports startup/socket/cleanup functions."
  (let* ((bytes (nelisp-pe-write-test--emit-exe 'winsock-socket-exit-42))
         (pe-off (nelisp-pe-write-test--read-le32 bytes #x3c))
         (opt-off (+ pe-off 4 20))
         (import-dir-off (+ opt-off 112 8))
         ...
         (socket-hint-rva
          (nelisp-pe-write-test--read-le64
           bytes (+ idata-raw (- w-first-thunk-rva idata-rva) 8)))
         (closesocket-hint-rva
          (nelisp-pe-write-test--read-le64
           bytes (+ idata-raw (- w-first-thunk-rva idata-rva) 16)))
         (wsacleanup-hint-rva
          (nelisp-pe-write-test--read-le64
           bytes (+ idata-raw (- w-first-thunk-rva idata-rva) 24))))
    (should (= import-rva #x3000))
    (should (= import-size 60))
    (should (= iat-rva #x3074))
    (should (= iat-size 56))
    (dotimes (i 20)
      (should (= (aref bytes (+ null-desc i)) 0)))
    (should (equal (nelisp-pe-write-test--read-c-string bytes k-name-off)
                   "KERNEL32.dll"))
    (should (equal (nelisp-pe-write-test--read-c-string bytes w-name-off)
                   "WS2_32.dll"))
    (should (equal (nelisp-pe-write-test--read-c-string
                    bytes (+ idata-raw (- exit-hint-rva idata-rva) 2))
                   "ExitProcess"))
    (should (equal (nelisp-pe-write-test--read-c-string
                    bytes (+ idata-raw (- wsa-hint-rva idata-rva) 2))
                   "WSAStartup"))
    (should (equal (nelisp-pe-write-test--read-c-string
                    bytes (+ idata-raw (- socket-hint-rva idata-rva) 2))
                   "socket"))
    (should (equal (nelisp-pe-write-test--read-c-string
                    bytes (+ idata-raw (- closesocket-hint-rva idata-rva) 2))
                   "closesocket"))
    (should (equal (nelisp-pe-write-test--read-c-string
                    bytes (+ idata-raw (- wsacleanup-hint-rva idata-rva) 2))
                   "WSACleanup"))))

(ert-deftest nelisp-pe-write-exe-binary-winsock-socket-entry-code ()
  "The Winsock socket smoke EXE sets Win64 args, closes, and cleans up."
  (let* ((bytes (nelisp-pe-write-test--emit-exe 'winsock-socket-exit-42))
         (text-off #x200))
    (should (equal (substring bytes text-off (+ text-off 4))
                   (unibyte-string #x48 #x83 #xec #x28)))
    ...
    (should (equal (substring bytes (+ text-off 16) (+ text-off 18))
                   (unibyte-string #xff #x15)))
    (should (equal (substring bytes (+ text-off 22) (+ text-off 26))
                   (unibyte-string #x85 #xc0 #x75 #x3e)))
    (should (= (aref bytes (+ text-off 26)) #xb9))
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ text-off 27)) 2))
    (should (= (aref bytes (+ text-off 31)) #xba))
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ text-off 32)) 1))
    (should (equal (substring bytes (+ text-off 36) (+ text-off 38))
                   (unibyte-string #x41 #xb8)))
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ text-off 38)) 6))
    (should (equal (substring bytes (+ text-off 42) (+ text-off 44))
                   (unibyte-string #xff #x15)))
    (should (equal (substring bytes (+ text-off 48) (+ text-off 54))
                   (unibyte-string #x48 #x83 #xf8 #xff #x74 #x22)))
    (should (equal (substring bytes (+ text-off 54) (+ text-off 57))
                   (unibyte-string #x48 #x89 #xc1)))
    (should (equal (substring bytes (+ text-off 57) (+ text-off 59))
                   (unibyte-string #xff #x15)))
    (should (equal (substring bytes (+ text-off 63) (+ text-off 67))
                   (unibyte-string #x85 #xc0 #x75 #x15)))
    (should (equal (substring bytes (+ text-off 67) (+ text-off 69))
                   (unibyte-string #xff #x15)))
    (should (equal (substring bytes (+ text-off 73) (+ text-off 77))
                   (unibyte-string #x85 #xc0 #x75 #x0b)))
    (should (= (aref bytes (+ text-off 77)) #xb9))
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ text-off 78)) 42))
    (should (equal (substring bytes (+ text-off 82) (+ text-off 84))
                   (unibyte-string #xff #x15)))
    (should (= (aref bytes (+ text-off 88)) #xb9))
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ text-off 89)) 1))
    (should (equal (substring bytes (+ text-off 93) (+ text-off 95))
                   (unibyte-string #xff #x15)))
    (should (= (aref bytes (+ text-off 99)) #xcc))))
...
(ert-deftest nelisp-pe-write-exe-binary-getfiletype-section-table ()
  "The GetFileType smoke EXE has path rdata, HANDLE data, and imports."
  (let* ((bytes (nelisp-pe-write-test--emit-exe 'getfiletype-exit-42))
         (pe-off (nelisp-pe-write-test--read-le32 bytes #x3c))
         (file-off (+ pe-off 4))
         (opt-off (+ file-off 20))
         (sect0 (+ pe-off 4 20 240))
    ...
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ sect1 20)) rdata-raw))
    (should (string-prefix-p ".data" (substring bytes sect2 (+ sect2 8))))
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ sect2 8)) 8))
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ sect2 12)) #x3000))
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ sect2 20)) data-raw))
    (should (string-prefix-p ".idata" (substring bytes sect3 (+ sect3 8))))
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ sect3 12)) #x4000))
...
(ert-deftest nelisp-pe-write-exe-binary-getfiletype-import-directory ()
  "The GetFileType smoke EXE imports file type lifecycle APIs."
  (let ((bytes (nelisp-pe-write-test--emit-exe 'getfiletype-exit-42)))
    (dolist (name '("KERNEL32.dll"
                    "ExitProcess"
                    "CreateFileW"
...
(ert-deftest nelisp-pe-write-exe-binary-getfiletype-entry-code ()
  "The GetFileType smoke EXE checks FILE_TYPE_DISK and cleans up."
  (let* ((bytes (nelisp-pe-write-test--emit-exe 'getfiletype-exit-42))
         (pe-off (nelisp-pe-write-test--read-le32 bytes #x3c))
         (sect0 (+ pe-off 4 20 240))
         (text-size (nelisp-pe-write-test--read-le32 bytes (+ sect0 8)))
    ...
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x48 #xc7 #x44 #x24 #x30
                                   #x00 #x00 #x00 #x00)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x48 #x83 #xf8 #xff #x74 #x42)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x48 #x89 #x05)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x48 #x89 #xc1)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x83 #xf8 #x01 #x75 #x2d)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x85 #xc0 #x74 #x1c)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x85 #xc0 #x74 #x0b)))
...
(ert-deftest nelisp-pe-write-exe-binary-getfileinformation-section-table ()
  "The GetFileInformationByHandle smoke EXE has path and info storage."
  (let* ((bytes (nelisp-pe-write-test--emit-exe
    ...
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ sect1 20)) rdata-raw))
    (should (string-prefix-p ".data" (substring bytes sect2 (+ sect2 8))))
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ sect2 8)) 60))
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ sect2 12)) #x3000))
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ sect2 20)) data-raw))
    (should (string-prefix-p ".idata" (substring bytes sect3 (+ sect3 8))))
    (should (= (nelisp-pe-write-test--read-le32 bytes (+ sect3 12)) #x4000))
...
(ert-deftest nelisp-pe-write-exe-binary-getfileinformation-import-directory ()
  "The GetFileInformationByHandle smoke EXE imports file information APIs."
  (let ((bytes (nelisp-pe-write-test--emit-exe
                'getfileinformation-exit-42)))
    (dolist (name '("KERNEL32.dll"
                    "ExitProcess"
...
(ert-deftest nelisp-pe-write-exe-binary-getfileinformation-entry-code ()
  "The GetFileInformationByHandle smoke EXE queries info and cleans up."
  (let* ((bytes (nelisp-pe-write-test--emit-exe
                 'getfileinformation-exit-42))
         (pe-off (nelisp-pe-write-test--read-le32 bytes #x3c))
         (sect0 (+ pe-off 4 20 240))
    ...
    (should (equal (substring bytes (+ text-off 4) (+ text-off 7))
                   (unibyte-string #x48 #x8d #x0d)))
    (should (equal (substring bytes (+ text-off 11) (+ text-off 16))
                   (unibyte-string #xba #x00 #x00 #x00 #xc0)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x45 #x31 #xc0 #x45 #x31 #xc9)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #xc7 #x44 #x24 #x20
    ...
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x48 #xc7 #x44 #x24 #x30
                                   #x00 #x00 #x00 #x00)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x48 #x83 #xf8 #xff #x74 #x48)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x48 #x89 #x05)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x48 #x89 #xc1)))
...
(ert-deftest nelisp-pe-write-exe-binary-filemapping-section-table ()
  "The file mapping smoke EXE has path and HANDLE/view storage."
  (let* ((bytes (nelisp-pe-write-test--emit-exe 'filemapping-exit-42))
         (pe-off (nelisp-pe-write-test--read-le32 bytes #x3c))
         (file-off (+ pe-off 4))
         (opt-off (+ file-off 20))
         (sect0 (+ pe-off 4 20 240))
...
(ert-deftest nelisp-pe-write-exe-binary-filemapping-import-directory ()
  "The file mapping smoke EXE imports mapping lifecycle APIs."
  (let ((bytes (nelisp-pe-write-test--emit-exe 'filemapping-exit-42)))
    (dolist (name '("KERNEL32.dll"
                    "ExitProcess"
                    "CreateFileW"
...
(ert-deftest nelisp-pe-write-exe-binary-filemapping-entry-code ()
  "The file mapping smoke EXE maps, unmaps, closes, and deletes."
  (let* ((bytes (nelisp-pe-write-test--emit-exe 'filemapping-exit-42))
         (pe-off (nelisp-pe-write-test--read-le32 bytes #x3c))
         (sect0 (+ pe-off 4 20 240))
         (text-size (nelisp-pe-write-test--read-le32 bytes (+ sect0 8)))
    ...
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #xc7 #x44 #x24 #x20
                                   #x02 #x00 #x00 #x00)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x48 #x83 #xf8 #xff #x75 #x0b)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x48 #x89 #x05)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x31 #xd2 #x41 #xb8
                                   #x04 #x00 #x00 #x00)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #xc7 #x44 #x24 #x20
                                   #x00 #x10 #x00 #x00)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #xba #x02 #x00 #x00 #x00)))
    (should (nelisp-pe-write-test--contains-p
             bytes (unibyte-string #x48 #xc7 #x44 #x24 #x20
                                   #x00 #x10 #x00 #x00)))
    (should (nelisp-pe-write-test--contains-p
