...
(ert-deftest nelisp-stdlib-string-byte-at-raw-bytes ()
  (should (= (nelisp--string-byte-at (unibyte-string 208 0 255) 0) 208))
  (should (= (nelisp--string-byte-at (unibyte-string 208 0 255) 1) 0))
  (should (= (nelisp--string-byte-at (unibyte-string 208 0 255) 2) 255))
...
(ert-deftest nelisp-stdlib-base64-raw-byte-parity ()
  (let* ((payload (unibyte-string 208 0 255 65 66))
         (expected (base64-encode-string payload)))
    (should (equal expected
                   (let ((alphabet "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/")
                         ...
                         (part-count 0))
                     (while (< i len)
                       (let* ((a (nelisp--string-byte-at payload i))
                              (have-b (< (1+ i) len))
                              (have-c (< (+ i 2) len))
                              (b (if have-b (nelisp--string-byte-at payload (1+ i)) 0))
                              (c (if have-c (nelisp--string-byte-at payload (+ i 2)) 0))
