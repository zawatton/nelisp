...
(ert-deftest nelisp-stdlib-os-wait-status-decoders ()
  "POSIX wait status helpers decode normal exits and signal termination."
  (should (nelisp-os-WIFEXITED #x2a00))
  (should (= (nelisp-os-WEXITSTATUS #x2a00) 42))
  (should (= (nelisp-os-WEXITSTATUS #xff00) 255))
  (should-not (nelisp-os-WIFSIGNALED #x2a00))
  (should-not (nelisp-os-WIFEXITED 15))
  (should (nelisp-os-WIFSIGNALED 15))
  (should (= (nelisp-os-WTERMSIG 15) 15))
  (should-not (nelisp-os-WIFSIGNALED 127)))

(ert-deftest nelisp-stdlib-os-byte-helpers-read-write-at ()
  "Raw byte helpers preserve offsets and unibyte payloads."
  (let (writes)
    (cl-letf (((symbol-function 'ptr-read-u8)
               (lambda (ptr off)
                 (pcase (list ptr off)
                   ...
                   (`(100 3) ?B)
                   (`(100 4) 255)
                   (_ (error "unexpected ptr-read-u8 %S %S" ptr off)))))
              ((symbol-function 'ptr-write-u8)
               (lambda (ptr off byte)
                 (push (list ptr off byte) writes)
                 byte)))
      (let ((expected (make-string 3 0)))
        (aset expected 0 ?A)
        (aset expected 1 ?B)
        (aset expected 2 255)
        (should (equal (nelisp-os--read-bytes-at 100 2 3)
                       expected)))
      (let ((payload (make-string 3 0)))
        (aset payload 0 ?X)
        (aset payload 1 ?Y)
        (aset payload 2 255)
        (should (= (nelisp-os--write-bytes-at 200 5 payload)
                   3))))
    (should (equal (nreverse writes)
                   '((200 5 88) (200 6 89) (200 7 255))))))
...
(ert-deftest nelisp-stdlib-os-sendmsg-fds-darwin-uses-libc-sendmsg ()
  "Darwin sendmsg-fds builds SCM_RIGHTS control messages and calls libc sendmsg."
  (let ((call nil)
        (writes nil)
        (payload nil)
        (freed nil)
...
(ert-deftest nelisp-stdlib-os-recvmsg-fds-darwin-uses-libc-recvmsg ()
  "Darwin recvmsg-fds reads SCM_RIGHTS control messages from libc recvmsg."
  (let ((call nil)
        (writes nil)
        (read-payload nil)
        (freed nil)
...
(ert-deftest nelisp-stdlib-os-getsockopt-peercred-darwin-uses-local-options ()
  "Darwin peer credentials use SOL_LOCAL LOCAL_PEERPID and LOCAL_PEERCRED."
  (let ((calls nil)
        (writes nil)
        (freed nil)
        (allocs '(3000 4000 5000 6000)))
    (cl-letf (((symbol-function 'nelisp-os--alloc) (lambda (_n) (pop allocs)))
              ...
              ((symbol-function 'nelisp-os-read-i32)
               (lambda (ptr off)
                 (pcase (list ptr off)
                   (`(3000 0) 1234)
                   (_ (error "unexpected peercred i32 read %S %S" ptr off)))))
              ((symbol-function 'nelisp-os-read-u32)
               (lambda (ptr off)
                 (pcase (list ptr off)
                   (`(5000 4) 501)
                   (`(5000 12) 20)
                   (_ (error "unexpected peercred u32 read %S %S" ptr off)))))
              ((symbol-function 'nelisp-os--libc-call)
               (lambda (dll fn sig &rest args)
                 (push (list dll fn sig args) calls)
                 0)))
      (let ((system-type 'darwin))
        (should (equal (nelisp-os-getsockopt-peercred 7)
                       '(1234 501 20)))))
    (should (equal (nreverse calls)
                   (list
                    (list "libc" "getsockopt"
                          [:sint32 :sint32 :sint32 :sint32 :pointer :pointer]
                          (list 7 nelisp-os-SOL-LOCAL
                                nelisp-os-LOCAL-PEERPID 3000 4000))
                    (list "libc" "getsockopt"
                          [:sint32 :sint32 :sint32 :sint32 :pointer :pointer]
                          (list 7 nelisp-os-SOL-LOCAL
                                nelisp-os-LOCAL-PEERCRED 5000 6000)))))
    (should (equal (nreverse writes)
                   (list
                    (list 4000 0 4)
                    (list 6000 0 nelisp-os--darwin-xucred-len))))
    (should (equal (sort freed #'<) '(3000 4000 5000 6000)))))
